  ...
  switch (typeof value) {
    ...
    case 'object':
      ...
      if (value instanceof ObjectId) {
        const objectIdStr = value.toString();
        
        length = utf8Length(objectIdStr);
        
        if (length < 0x100) { // str 8
          bytes.push(0xe4, length);
          size = 2;
        } else {
          throw new Error('ObjectId too long');
        }
        defers.push({ str: objectIdStr, length: length, offset: bytes.length });
        return size + length;
      }
